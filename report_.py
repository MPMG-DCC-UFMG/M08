...
#             self.results['Tempo de Análise'].append(round(result['Tempo de Análise'], 2))
    ...
    def generate_vid_summary(self, return_path=True):
        ...
        self.results  = {'Arquivo': [], 'NSFW': [], 'Faces': [], 'Confiança Faces': [],  
                         'Idades': [], 'Criança': [], 'Classe': [], 'Tempo de Análise': []}
        ...
        for result in results:
            ...
            self.results['NSFW'].append(prob_nsfw)
            
            num_faces = self.get_unique(result['frames_video'], self.res_order.index('cont_age'))
            self.results['Faces'].append({'Média': round(np.mean(num_faces), 1), 
            ...
            self.results['Idades'].append([self.ageclasses[age] for age in ages])
            ...
            self.results['Criança'].append([False if child == 1 else True for child in conf_child])
