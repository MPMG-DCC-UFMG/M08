...
class Log():
    ...
    def __init__(self, id_analysis=None, rootpath='', std_out=False):

        self.results  = {'images': [], 'videos': [], 'rootpath': rootpath}
        self.buffer  = ''
        ...
        if id_analysis is not None:
            self.set_id(id_analysis)
        else:
            self.log_path = self.log_root
